...
export default function PanolApp() {
  ...
  const [rec, setRec] = useState<RecepcionForm>({
    remito: '',
    sap: ''
  });
  ...
  const [vale, setVale] = useState<ValeForm>({
    nro: 'V-001',
    frente: 'Campo',
    sap: '',
    cantidad: '',
  ...
  const [trazabilidad, setTrazabilidad] =
    useState<TrazabilidadForm>({
      fecha: '',
      linea: '',
      spool: '',
      junta: '',
  ...
  async function handleGuardarRecepcion(
  ...
  async function handleGuardarVale(
    e: React.FormEvent<HTMLFormElement>
  ) {
    e.preventDefault();
    ...
    try {
      ...
      alert(
        '¡Vale de salida registrado con éxito!'
      );
  ...
  async function fetchInventarioRealTime() {
    try {
      ...
      if (materialesResult.error) {
        throw materialesResult.error;
      }

      if (recepcionesResult.error) {
        throw recepcionesResult.error;
      }

      if (valesResult.error) {
        throw valesResult.error;
      }

      const mats =
        (materialesResult.data || []) as Material[];

      const recs =
        (recepcionesResult.data || []) as RecepcionDB[];

      const vales =
        (valesResult.data || []) as ValeDB[];

      // --------------------------------------------------------
      // CALCULAR STOCK
      // --------------------------------------------------------

      const stockCalculado: InventarioItem[] =
        mats.map((m) => {

          const ingresado = recs
            .filter(
              (r) =>
                r.codigo_sap === m.codigo_sap
            )
            .reduce(
              (acc, r) =>
                acc +
          ...
          const entregado = vales
            .filter(
              (v) =>
                v.codigo_sap === m.codigo_sap
            )
            .reduce(
              (acc, v) =>
                acc +
  ...
  async function handleGuardarTrazabilidad(
    e: React.FormEvent<HTMLFormElement>
  ) {
    e.preventDefault();
    ...
    try {
      ...
      alert(
        '¡Trazabilidad de junta registrada con éxito!'
      );
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm">
          ...
          <h1 className="text-xl font-bold text-slate-900 capitalize">
            ...
            {seccion === 'recepcion' &&
              'Recepción de Materiales y Certificados'}

            {seccion === 'vales' &&
              'Vales de Salida al Frente'}

            {seccion === 'inventario' &&
              'Inventario Permanente y Stock'}

            {seccion === 'trazabilidad' &&
              'Trazabilidad de Juntas y Spools'}
        ...
        <div className="p-8">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                <h3 className="text-md font-bold text-slate-800 mb-4">
                  ➕ Dar de alta nuevo ítem en el Catálogo
                </h3>

                <form
                  onSubmit={handleCrearMaterial}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                  ...
                  <h3 className="font-bold text-slate-800">
                    Listado Maestro Actual ({materiales.length})
                  </h3>
          ...
          {seccion === 'recepcion' && (
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 max-w-lg">
              ...
              <form
                onSubmit={handleGuardarRecepcion}
                className="space-y-4"
              >
                ...
                <button
                  type="submit"
                  disabled={subiendo}
                  className="w-full bg-emerald-600 text-white font-bold py-2 rounded-lg hover:bg-emerald-700 transition disabled:opacity-50"
                >
                  {subiendo
          ...
          {seccion === 'vales' && (
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 max-w-lg">
              ...
              <h3 className="font-bold text-slate-800 mb-4">
                📄 Registrar Vale de Salida al Frente
              </h3>
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    placeholder="Ej: Campo / Taller"
                    value={vale.frente}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setVale({
                        ...vale,
                        frente: e.target.value
                      })
                    }
                  />
          ...
          {seccion === 'inventario' && (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                ...
                <button
                  onClick={fetchInventarioRealTime}
                    ...
                    {inventario.length === 0 ? (
                      ...
                      inventario.map((item) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-6 text-slate-700 font-semibold">
                            {item.ingresado}
                          </td>
          ...
          {seccion === 'trazabilidad' && (
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 max-w-xl">
              ...
              <h3 className="font-bold text-slate-800 mb-2">
                🔗 Registro de Trazabilidad (Junta / Spool / Colada)
              </h3>

              <p className="text-xs text-slate-500 mb-4">
                Obligatorio para el dossier de calidad.
                Cada junta debe vincularse con su certificado.
              </p>

              <form
                onSubmit={handleGuardarTrazabilidad}
                className="space-y-4"
              >
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="Ej: P1-J-001"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                      value={trazabilidad.junta}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setTrazabilidad({
                          ...trazabilidad,
                          junta: e.target.value
                        })
                      }
                    />
                ...
                <div>
                  ...
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                    Material Empleado (Descripción)
                  </label>

                  <input
                    type="text"
                    required
                    placeholder="Ej: Caño 4in A312 TP316L"
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    value={trazabilidad.material}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setTrazabilidad({
                        ...trazabilidad,
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                      Nro de Colada (Material)
                    </label>

                    <input
                      type="text"
                      required
                      placeholder="Ej: H-55107"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                      value={trazabilidad.colada}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setTrazabilidad({
                          ...trazabilidad,
                          colada: e.target.value
                        })
                      }
                    />
                  ...
                  <div>
                    ...
                    <label className="block text-xs font-semibold text-slate-600 mb-1">
                      Colada de Aporte / Gas
                    </label>

                    <input
                      type="text"
                      required
                      placeholder="Ej: C-88214"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                      value={trazabilidad.aporte}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setTrazabilidad({
                          ...trazabilidad,
                ...
                <button
                  type="submit"
                  disabled={guardandoTraz}
                  className="w-full bg-blue-600 text-white font-bold py-2 rounded-lg hover:bg-blue-700 transition disabled:opacity-50"
                >
                  {guardandoTraz
                    ? 'Registrando trazabilidad...'
                    : 'Guardar Trazabilidad de Junta'}
                </button>
          ...
          {/* ==================================================
              DASHBOARD / MÓDULOS EN CONSTRUCCIÓN
          ================================================== */}
